...
const TEMP_DIR = path.join(EXPORT_DIR, '___temp');

function run() {
  console.log(`\nAttempting to fix exports in ${EXPORT_DIR}\n`);
  fs.readdir(EXPORT_DIR, (error, contents) => {
    if(error) {
      return exit(error);
    }
...
function unzip(filename, unzipdir, cb) {
  new admzip(path.join(EXPORT_DIR, filename)).extractAllTo(unzipdir);
  cb();
}
...
function zip(dir, newZipName, cb) {
  ...
  zip.writeZip(path.join(EXPORT_DIR, newZipName));
  cb();
}

function exit(error) {
  fs.remove(TEMP_DIR).then(_handleExit).catch(_handleExit);
